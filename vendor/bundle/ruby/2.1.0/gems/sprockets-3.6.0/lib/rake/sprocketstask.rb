...
module Rake
  ...
  class SprocketsTask < Rake::TaskLib
    ...
    # Define tasks
    def define
      desc name == :reservations ? "Compile assets" : "Compile #{name} assets"
      task name do
        with_logger do
      ...
      desc name == :reservations ? "Remove all assets" : "Remove all #{name} assets"
      ...
      desc name == :reservations ? "Clean old assets" : "Clean old #{name} assets"
      task "clean_#{name}" do
        with_logger do
          manifest.clean(keep)
        end
